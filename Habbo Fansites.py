...
@slash.slash(
    name="fansites", description="Fansites Habbo Hotel",
    options=[
...
async def _fansites(ctx:SlashContext, hotel:str):
    await ctx.defer()
    ...
    url = f"https://images.habbo.com/habbo-web-news/{HotelNoticia}/production/fansites_1.html"
    page = requests.get(url)
    soup = BeautifulSoup(page.content, 'html.parser')

    hotel=f"{hotel}" ## es -> com ->it -> de -> fr -> com.tr ->nl -> com.br

    titulo = soup.find_all('h2', class_='news-header__title')[0].text

    imagen = soup.find_all('img')[0]
    imagenPeque = soup.find_all('img')[0]

    urlNoticia = soup.find_all('a', class_="news-header__link news-header__banner")[0]

    descripcion = soup.find_all('p', class_='news-header__wrapper news-header__summary')[0].text 
    #####
